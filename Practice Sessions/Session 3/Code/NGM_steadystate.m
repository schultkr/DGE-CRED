function [ys,params,check] = NGM_steadystate(ys,exo,M_,options_)
...
if inbCV_p > 0
    for icocv = 1:inbCV_p
       temp = temp + eval(['CV_' num2str(icocv)]) * eval(['theta_' num2str(icocv) '_p']);
    end
end
D = 1 - exp(-temp);
k = ((1-beta_p*(1-delta_p))/(beta_p*alpha_p * (1 - D) *A))^(1/(alpha_p-1));
c = (1 - D) * A*k^alpha_p-delta_p*k;
lamb = c^(-sigma_p);
% ... steady state is now determined.
