...
@# for reg in 1:Regions
    [name = 'Temperature']
    //T_@{reg} = rhoT_p * T_@{reg}(-1) + (1 - rhoT_p) * (T0_@{reg}_p + exo_T_@{reg});
    T_@{reg} = T_@{reg}(-1) + min(0.072,(T0_@{reg}_p + exo_T_@{reg} - T_@{reg}(-1)));

    [name = 'Wind speed']
    WS_@{reg} = rhoWS_p * WS_@{reg}(-1) + (1 - rhoWS_p) * (WS0_@{reg}_p + exo_WS_@{reg});

    [name = 'Percipitation']
    PERC_@{reg} = rhoPERC_p * PERC_@{reg}(-1) + (1 - rhoPERC_p) * (PERC0_@{reg}_p + exo_PERC_@{reg});
@# endfor
[name = 'Sea level']
SL = rhoSL_p * SL(-1) + (1 - rhoSL_p) * (SL0_p + exo_SL);
