...
@# for sec in 1:Sectors
    ...
    @# for subsec in Subsecstart[sec]:Subsecend[sec]
        ...
        @# for reg in 1:Regions
            ...
            tauKF_@{subsec}_@{reg} ${\tau^{K}_{s,r}}$ (long_name = 'regional sector corporate tax rate on capital')
            tauNF_@{subsec}_@{reg} ${\tau^{N}_{s,r}}$ (long_name = 'regional sector labour tax rate on capital')
            P_D_@{subsec}_@{reg} ${P^D_{s,r}}$ (long_name = 'regional sector price index')
        @# endfor
    @# endfor
@# endfor
...
tauH  ${\tau^{H}}$ (long_name = 'tax on housing')
...
I_AP_DH ${I^{A,D^H}}$ (long_name = 'adaptation private expenditure for housing')       
@# for sec in 1:Sectors
    ...
    @# for subsec in Subsecstart[sec]:Subsecend[sec]
        ...
        @# for reg in 1:Regions
            ...
            D_N_@{subsec}_@{reg} ${D^N_{s,r}}$ (long_name = 'regional subsector damages to labour productivity')
            D_K_@{subsec}_@{reg} ${D^K_{s,r}}$ (long_name = 'regional subsector destuction of capital stock')
            K_@{subsec}_@{reg} ${K_{s,r}}$ (long_name = 'regional subsector capital')
            I_AP_@{subsec}_@{reg} ${I^{A,P}_{s,r}}$ (long_name = 'regional subsector private adaptation government expenditure')       
...
@# for sec in 1:Sectors
    @# for subsec in Subsecstart[sec]:Subsecend[sec]
        ...
        @# for reg in 1:Regions
            ...
            exo_GA_@{subsec}_@{reg} ${\eta_{G^{A},s,r}}$ (long_name = 'exogenous sector adaptation expenditure against @{z}')
            exo_IAP_@{subsec}_@{reg} ${\eta_{I^{A,P},s,r}}$ (long_name = 'exogenous sector private adaptation expenditure against @{z}')
        @# endfor
    @# endfor
@# endfor
exo_G_A_DH ${\eta_{G^{A,H}}}$ (long_name = 'exogenous sector adaptation expenditure for housing')
exo_I_AP_DH ${\eta_{G^{A,P,H}}}$ (long_name = 'exogenous sector private adaptation expenditure for housing')
@# for z in ClimateVarsNational
...
@# for sec in 1:Sectors
    ...
    @# for subsec in Subsecstart[sec]:Subsecend[sec]
        iGA_@{subsec}_p ${i^{G^A}_s}$ (long_name = 'subsector which provides goods for adaptation measures in the respecitve sub-sector')
        iIAP_@{subsec}_p ${i^{A,P}_s}$ (long_name = 'subsector which provides goods for private adaptation measures in the respecitve sub-sector')
        ...
        etaQ_@{subsec}_p ${\eta^{Q}_{k}}$ (long_name = 'elasticity of substitution between regional production')
        etaI_@{subsec}_p ${\eta^{I}_{k}}$ (long_name = 'elasticity of substitution between value added and intermediate products')
        @# for reg in 1:Regions
...
iGAH_p ${i^{G^{A,H}}}$ (long_name = 'government housing expenditure sector')
iIAPH_p ${i^{A,P,H}}$ (long_name = 'sub sector output used for private housing adaptation expenditure')
...
@# for reg in 1:Regions
    @# for z in ClimateVarsRegional
        @{z}0_@{reg}_p ${@{z}_{0,r}}$ (long_name = 'initial regional @{z}')
    @# endfor
@# endfor
% === initial values ===
...
P0_M_p ${P^M_0}$ (long_name = 'initial price level')
N0_p ${N_0}$ (long_name = 'initial employment')
