function [fval_vec,strys,strexo] = FindK(x,strys,strexo,strpar)
    ...
    if strpar.iIAPH_p == 0
        ...
        strys.I_AP_DH = strexo.exo_I_AP_DH * strpar.Y0_p / (strpar.P0_p * strpar.(['P_D_' num2str(strpar.iIAPH_p) '_p']));    
    end    
    ...
    for icosec = 1:strpar.inbsectors_p
        ...
        for icosubsec = strpar.(['substart_' ssec '_p']):strpar.(['subend_' ssec '_p'])
            ...
            strys.(['Q_D_' ssubsec]) = strys.(['Q_' ssubsec]) - strys.(['X_' ssubsec]) - (strpar.iIAPH_p == icosubsec) * strys.I_AP_DH * strys.P - (strpar.iGAH_p == icosubsec) * strys.G_A_DH * strys.P  - strys.(['GA_direct_' ssubsec]) * strys.P;
